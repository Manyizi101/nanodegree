...
// default to live urls
var underbarServerURL = 'http://oi-underbar.oup.com/underbar/';
var underbarAssetURL = 'http://oi-underbar.oup.com/underbar/';
var prefixRelatedContentURL = 'http://oxfordindex.oup.com/search?';

// These two global variables (gUnderbarBaseUrl, gUnderbarAssetUrl, gOxfordIndexBaseUrl) are defined in the
// UnderbarHostsJS javascript file which is dynamically rendered from the underbar server
// and whose values are driven by the configuration of that app.
if (typeof gUnderbarBaseUrl !== 'undefined') {
    underbarServerURL = gUnderbarBaseUrl;
}
if (typeof gUnderbarAssetUrl !== 'undefined') {
    underbarAssetURL = gUnderbarAssetUrl;
}
if (typeof gOxfordIndexBaseUrl !== 'undefined') {
    prefixRelatedContentURL = gOxfordIndexBaseUrl + 'search?';
}

var resourceURL = underbarAssetURL;
...
// #1308 SiQ event tagging: need to unset all the custom NTPT_PGEXTRA data set by the host
if (typeof NTPT_PGEXTRA !== 'undefined') {
	NTPT_PGEXTRA = undefined;
}

// #1308 need to pull in SiQ page tag javascript (if not already done in host)
if (typeof(ntptEventTag) !== "function") {
	// OUP Platform #361: don't use getScript() as it adds a timestamp parameter to the request
	// which NetInsight does not like.  This is equivalent to doing getScript() but enforces caching
	jQuery.ajax({
		  url: 'http://ouptag.scholarlyiq.com/ntpagetag.js',
		  dataType: "script",
		  complete: function(jqXHR, textStatus) { 
			  console.log("Completed fetching ntpagetag.js from underbar.  Status: " + textStatus) },
		  cache: true
		});
}
...
/*
 * #1308 Register an event with Scholarly iQ.  Note siteid and supplier are hard-coded.
 */
function registerSiQEvent(event) {
	ntptEventTag(
			'event_type='
			+ event 
			+ '&host_site=' 
			+ qc_productCode 
			+ '&siteid=OI-Underbar&supplier_tag=Safari'
			);
}
...
function openOIUnderbar(type){	
	...
	if (type == 1){
		...
		// #1308 call ntptEventTag() to register SiQ event
		registerSiQEvent('RELATEDLINKS');
...
function openOIUnderbar_About(){	
	...
	// #1308 call ntptEventTag() to register event
	registerSiQEvent('ABOUTTHEINDEX');
...
function setContentColumns(value, contentType, i, mode, index){	
	...
	if(contentType == "Journal") {		
	    ...
	    ulContent += "<a title='"+encodeTitle(value.title)+"' href='" + gOxfordIndexBaseUrl + "view/" + value.doi + "' target='_blank'>" + titleTemp + "</a>";
